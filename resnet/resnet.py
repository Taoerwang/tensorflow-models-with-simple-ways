...
class Resnet:
    ...
    def resUnit(self, x, filters):
        n_in = x.get_shape()[-1]
        part1 = tf.layers.batch_normalization(x)
        part2 = tf.nn.relu(part1)
        part3 = tf.layers.conv2d(inputs=part2,
                                 filters=filters,
                                 kernel_size=[3, 3],
                                 padding="same")
        ...
        part6 = tf.layers.conv2d(inputs=part5,
                                 filters=filters,
                                 kernel_size=[3, 3],
                                 padding="same")
        if n_in != filters:
    ...
    def classifier(self, input, reuse=False):
        with tf.variable_scope('classifier', reuse):
            ...
            layer1 = tf.layers.max_pooling2d(inputs=layer1, pool_size=[3, 3], strides=1)

            layer1 = self.resUnit(layer1, 32)
            layer1 = self.resUnit(layer1, 32)
            layer1 = self.resUnit(layer1, 64)
            layer1 = self.resUnit(layer1, 64)
            layer1 = self.resUnit(layer1, 32)
            layer1 = self.resUnit(layer1, 32)

            pool = tf.layers.average_pooling2d(inputs=layer1, pool_size=[3, 3], strides=1)
            x = tf.reshape(pool, [-1, 24 * 24 * 32])
    ...
    def train(self, train_steps=10000, batch_size=100, learning_rate=0.0001, save_model_numbers=3):
