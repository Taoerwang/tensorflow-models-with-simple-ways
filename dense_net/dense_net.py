...
class DenseNet:
    ...
    def denseBlock(self, x, filters):
        ...
        for i in range(3):
            ...
            layer = tf.layers.conv2d(inputs=layer,
                                     ...
                                     kernel_size=[3, 3],
                                     padding="same")
            x = tf.concat([layer, x], axis=3)
            dense_out.append(layer)
    ...
    def classifier(self, input, reuse=False):
        with tf.variable_scope('classifier', reuse):
            ...
            layer1 = self.denseBlock(layer1, 32)
            layer1 = tf.layers.conv2d(inputs=layer1,
                                      filters=32,
                                      kernel_size=[3, 3],
                                      padding="same")
            layer1 = tf.layers.max_pooling2d(inputs=layer1, pool_size=[2, 2], strides=2)

            layer1 = self.denseBlock(layer1, 64)
            layer1 = tf.layers.conv2d(inputs=layer1,
                                      filters=64,
                                      kernel_size=[3, 3],
                                      padding="same")
